...
В проекте используются транзакции для операций выдачи и возврата книги.

Транзакция нужна для того, чтобы несколько SQL-действий выполнились как одна операция.

### Выдача книги

При выдаче книги в одной транзакции выполняются два действия:

1. Создаётся новая запись в таблице `borrow_records`.
2. В таблице `books` поле `available` меняется на `false`.

Если оба действия успешны, выполняется `COMMIT`.

Если во время операции происходит ошибка, выполняется `ROLLBACK`, и изменения не сохраняются.

### Возврат книги

При возврате книги в одной транзакции выполняются два действия:

1. В таблице `borrow_records` поле `returned_at` заполняется текущим временем.
2. В таблице `books` поле `available` меняется на `true`.
